...
const STATUS_CONFIG = {
  ...
  resolved:  { label: 'Đã giải quyết',color: 'bg-purple-100 text-purple-700' },
...
// Modal báo tranh chấp / khiếu nại
function DisputeModal({ booking, onConfirm, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl border border-slate-100 animate-in zoom-in-95 duration-200">
        ...
        <h3 className="text-xl font-bold text-slate-900 text-center mb-2">Báo tranh chấp / Khiếu nại</h3>
        <p className="text-slate-500 text-center text-sm mb-5 leading-relaxed">
          Gửi báo cáo khiếu nại về lớp học <strong className="text-slate-800">{booking.subject}</strong> với gia sư <strong className="text-slate-800">{booking.tutor_name}</strong> đến Admin.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <textarea
              ...
              placeholder="Nhập chi tiết vấn đề (VD: Gia sư nghỉ không báo trước, dạy không đủ thời gian...)"
              className="w-full text-xs p-3 border border-slate-200 rounded-xl focus:ring-2 focus:ring-rose-400 focus:outline-none resize-none"
            />
          </div>
          ...
          <div className="flex flex-col gap-2 pt-2">
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3.5 bg-rose-600 hover:bg-rose-700 text-white font-bold rounded-2xl transition-all shadow-lg shadow-rose-100 disabled:opacity-50 flex items-center justify-center gap-2"
            >
...
export default function BookingHistoryPage() {
  ...
  const handleDisputeConfirm = async (id, reason) => {
    try {
      const res = await fetch(`${API_BASE}/student/bookings/${id}/dispute`, {
        method: 'PUT',
