...
export default function TutorEarningsTable() {
  ...
  const fetchTutors = useCallback(async () => {
    try {
      ...
      console.error('TutorEarningsTable fetch error:', err);
              ...
              {tutors.map((t) => {
                ...
                return (
                  <tr key={t.id} className="hover:bg-slate-50/80 transition-colors">
                    ...
                    <td className="py-4 px-6">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-extrabold ${getBadgeClass(t.balance)}`}>
  ...
  const getBadgeClass = (balance) => {
    if (balance >= 200000) return 'bg-emerald-50 text-emerald-700 border border-emerald-200';
    if (balance > 0) return 'bg-blue-50 text-blue-700 border border-blue-200';
