# Tài Liệu Triển Khai: Chức Năng Lịch Sử Thuê Gia Sư (Student)

Tài liệu này hướng dẫn từng bước để code chức năng hiển thị **Lịch sử thuê gia sư** tại trang `HiringHistoryPage.jsx`. Quá trình này tuân thủ đúng kiến trúc 3 lớp của Backend.

---

## 1. Cấu Trúc Backend Đã Có (Kiểm tra lại)
Chức năng này đã có sẵn API Backend, tuy nhiên Developer cần đọc hiểu cấu trúc để biết cách gọi và xử lý dữ liệu.

*   **Service Layer (`backend/services/bookingService.js`)**:
    Hàm `getStudentBookings(userId, status)` làm nhiệm vụ query vào bảng `bookings` kết hợp (`JOIN`) với bảng `tutors` để lấy thông tin gia sư, môn học, ngày giờ, và trạng thái.
*   **Controller Layer (`backend/controllers/studentBookingController.js`)**:
    Hàm `getMyBookings` nhận `userId` từ token, gọi Service và chuẩn hóa kết quả qua hàm `sendSuccess`.
*   **Route Layer (`backend/routes/studentBookings.js`)**:
    Bảo vệ bằng middleware `verifyUser` và định nghĩa endpoint `GET /api/student/bookings`.

**👉 Endpoint sử dụng ở Frontend:** `GET http://localhost:3001/api/student/bookings`

---

## 2. Triển Khai Frontend (`HiringHistoryPage.jsx`)

Mở file `frontend/src/pages/User/HiringHistoryPage.jsx` và thực hiện các bước sau:

### Bước 2.1: Import các hook và icons cần thiết
```javascript
import React, { useState, useEffect } from 'react';
import { History, AlertCircle, Calendar, Clock, User, XCircle, AlertTriangle } from 'lucide-react';
```

### Bước 2.2: Khai báo State và lấy danh sách lịch sử
Bên trong component `HiringHistoryPage`:
```javascript
...
### Bước 2.3: Helper function cho Trạng thái
Thêm một hàm nhỏ để map trạng thái tiếng Anh sang tiếng Việt có màu sắc:
```javascript
...
### Bước 2.4: Render Giao diện (Thay thế khối return)
Thay thế đoạn HTML tĩnh hiện tại bằng mã sau:
```jsx
  return (
...
- Hàm Hủy (`cancelBooking`) và Khiếu nại (`reportDispute`) đã có sẵn ở API Backend (`PUT /api/student/bookings/:id/cancel`). Dựa vào tài liệu này, bạn có thể tiếp tục gắn sự kiện `onClick` gọi các hàm đó vào các nút Hủy/Khiếu nại.
